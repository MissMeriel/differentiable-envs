...
if __name__ == "__main__":
    ...
    exp_root_dir = 'dexnet-loop-500-iter-grad-scale/'
        ...
        for object in tqdm(object_list,desc=f'from {dataset}',leave=False):
            ...
            for name in db.data_['datasets'][dataset]['objects'][object]['grasps']:
                ...
                try:
                    ...
                    minDist = qf.self_collision_min(mesh,unconnectivity=mesh_props,forceNormalDist=0)
                except Exception as e:
                    tqdm.write(f'failed to compute connectivity on {object}, saving for debug faces: {faces.shape} verts: {verts.shape}')
                    tqdm.write(str(e))
                    #
                    log = open(f'debug_mesh_attack/{object}_crash.txt', 'a')
                    log.write(str(e))
                    log.close()
                    ...
                    break
                try:
                    grasp_name = random.choice(list(db.data_['datasets'][dataset]['objects'][object]['grasps'][name]))
                except Exception as e:
                    tqdm.write(f'failed to random choice on {object}, saving for debug faces: {faces.shape} verts: {verts.shape}')
                    tqdm.write(str(e))
                    #
                    log = open(f'debug_mesh_attack/{object}_empty.txt', 'a')
                    log.write(str(e))
                    log.close()
                    break
                #grasp_name = 'grasp_21'
                if minDist < 1e-9:
                    open(f'debug_mesh_attack/{object}_collision.txt', 'a').close()
                else:
                    # try:
                    gc.collect()
                    ...
                    run1.attack(mesh=mesh, grasp=grasp_h, dir=f"{exp_root_dir}/{object}/cf-UP-mw-DOWN-coll-up/{grasp_name}", lr=lr0, momentum=mom, loss_alpha=None, method=[AttackMethod.CF_UP,AttackMethod.MW_DOWN,AttackMethod.SELF_COLLISION_UP])
                    run1.attack(mesh=mesh, grasp=grasp_h, dir=f"{exp_root_dir}/{object}/cf-DOWN-mw-UP-coll-up/{grasp_name}", lr=lr0, momentum=mom, loss_alpha=None, method=[AttackMethod.CF_DOWN,AttackMethod.MW_UP,AttackMethod.SELF_COLLISION_UP])
                            
                    run1.attack(mesh=mesh, grasp=grasp_h, dir=f"{exp_root_dir}/{object}/cf-GQCNN-DIFF-coll-up/{grasp_name}", lr=lr0, momentum=mom, loss_alpha=None, method=[AttackMethod.GQCNN_CF_DIFF,AttackMethod.SELF_COLLISION_UP])
                    run1.attack(mesh=mesh, grasp=grasp_h, dir=f"{exp_root_dir}/{object}/cf-UP-GQCNN-DOWN-coll-up/{grasp_name}", lr=lr0, momentum=mom, loss_alpha=None, method=[AttackMethod.CF_UP,AttackMethod.GQCNN_DOWN,AttackMethod.SELF_COLLISION_UP])
                    run1.attack(mesh=mesh, grasp=grasp_h, dir=f"{exp_root_dir}/{object}/cf-DOWN-GQCNN-UP-coll-up/{grasp_name}", lr=lr0, momentum=mom, loss_alpha=None, method=[AttackMethod.CF_DOWN,AttackMethod.GQCNN_UP,AttackMethod.SELF_COLLISION_UP])
                    ...
                    #     pytorch3d.io.save_obj(f'debug_mesh_attack/{object}.obj', mesh.verts_packed(),mesh.faces_packed())
                    #     log = open(f'debug_mesh_attack/{object}_{grasp_name}.txt', 'a')
                    #     log.write(str(e))
                    #     log.close()
                    #     graspObj.write_obj(f'debug_mesh_attack/{object}_{grasp_name}.obj', include_coordinate=True, include_line_o_action=True)
