...
if __name__ == "__main__":
    ...
    exp_root_dir = 'dexnet-loop-500-iter-full-hull+robust/'
        ...
        for object in tqdm(object_list,desc=f'from {dataset}',leave=False):
            ...
            for name in db.data_['datasets'][dataset]['objects'][object]['grasps']:
                ...
                try:
                    ...
                    tqdm.write(f'failed to random choice on {object}, saving for debug')
                    tqdm.write(str(e))
                    #
                ...
                if minDist < 1e-9:
                    ...
                    try:
                        ...
                        run1.attack(mesh=mesh, grasp=grasp_h, dir=f"{exp_root_dir}/{object}/oracle-grad-UP-minweight-DOWN/{grasp_name}", lr=lr0, momentum=mom, loss_alpha=None, method=[AttackMethod.ORACLE_GRAD_UP,AttackMethod.MINWEIGHT_DOWN])
                        run1.attack(mesh=mesh, grasp=grasp_h, dir=f"{exp_root_dir}/{object}/oracle-grad-DOWN-minweight-UP/{grasp_name}", lr=lr0, momentum=mom, loss_alpha=None, method=[AttackMethod.ORACLE_GRAD_DOWN,AttackMethod.MINWEIGHT_UP])
                                
                        run1.attack(mesh=mesh, grasp=grasp_h, dir=f"{exp_root_dir}/{object}/oracle-grad/{grasp_name}", lr=lr0, momentum=mom, loss_alpha=None, method=AttackMethod.ORACLE_GRAD)
                        run1.attack(mesh=mesh, grasp=grasp_h, dir=f"{exp_root_dir}/{object}/oracle-grad-DOWN/{grasp_name}", lr=lr0, momentum=mom, loss_alpha=None, method=AttackMethod.ORACLE_GRAD_DOWN)
                        run1.attack(mesh=mesh, grasp=grasp_h, dir=f"{exp_root_dir}/{object}/oracle-grad-UP/{grasp_name}", lr=lr0, momentum=mom, loss_alpha=None, method=AttackMethod.ORACLE_GRAD_UP)
