...
def test_select_grasp():
	...
	grasp.trans_world_to_im(camera=renderer1.camera)
...
def test_trans_world_to_im():
	...
	grasp = Grasp.read("grasp-dataset2/grasp-batch.json",device=device)
	grasp = grasp[0]
...
def test_batching():
	...
	gb_read = Grasp.read("grasp-dataset/grasp-batch.json",device=device)
	...
	assert gb2.depth.shape == torch.Size([3,1])
	...
	g0 = Grasp.read(files[0],device=device)
	assert g0.num_grasps() == 1
	g1 = Grasp.read(files[1],device=device)
	g2 = Grasp.read(files[2],device=device)
	g3 = Grasp.read(files[3],device=device)
...
def test_slicing():
	...
	assert gb.im_center.shape == torch.Size([4,2])
	assert gb.num_grasps() == 4
	g0 = Grasp.read("/tmp/data0.json",device=device)
	assert g0.num_grasps() == 1
	...
	assert g0 == g0
	assert gb2 == gb2
	g0_dup = Grasp.read("/tmp/data0.json",device=device)
	assert g0 == g0_dup
...
def test_random_grasps():
	...
	g_orig = Grasp.read("grasp-dataset/grasp-batch.json",device=device)[0]
	g = Grasp.read("grasp-dataset/grasp-batch.json",device=device)[0]
	...
	gb = Grasp.read("grasp-dataset/grasp-batch.json",device=device)[0:1]
